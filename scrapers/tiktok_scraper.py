...
class TikTokScraper(BaseScraper):
    ...
    def scrape_hashtag(self, hashtag: str, max_results: int = 100) -> List[Dict]:
        ...
        while len(videos_data) < max_results and stale_scrolls < max_stale_scrolls:
            # Wait for video container to load (TikTok uses data-e2e attributes, not IDs)
            container = self.wait_for_element(By.CSS_SELECTOR, '[data-e2e="challenge-item-list"]', timeout=10)
    ...
    def _extract_videos_from_page(self, soup: BeautifulSoup, hashtag: str) -> List[Dict]:
        ...
        Uses TikTok's data-e2e attributes: challenge-item-list and challenge-item
        """
        videos = []

        # Find parent container using data-e2e attribute
        parent_container = soup.find(attrs={"data-e2e": "challenge-item-list"})

        if not parent_container:
            logging.warning("Parent container [data-e2e='challenge-item-list'] not found")
            return videos

        # Find all video containers using data-e2e="challenge-item"
        video_containers = parent_container.find_all(attrs={"data-e2e": "challenge-item"})
    ...
    def _extract_caption(self, container) -> str:
        ...
        try:
            # TikTok uses data-e2e attributes for content

            # Try challenge-item-desc (the standard selector)
            caption_elem = container.find(attrs={"data-e2e": "challenge-item-desc"})
            if caption_elem:
                return caption_elem.get_text(strip=True)

            # Fallback: Try search-card-desc
    ...
    def _extract_views(self, container) -> int:
        ...
        try:
            ...
            data_e2e_attrs = [
                "challenge-vvcount",  # Primary selector for hashtag pages
